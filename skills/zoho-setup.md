...
```bash
ls -la ~/.config/e2e/agents/.env 2>/dev/null
```
...
```bash
mkdir -p ~/.config/e2e/agents
chmod 700 ~/.config/e2e/agents
```

**Install the Zoho Sprint CLI script**:

```bash
curl -o ~/.config/e2e/agents/zoho-sprint.mjs \
  https://raw.githubusercontent.com/e2eneworks-oss/skills/main/scripts/zoho-sprint.mjs
chmod +x ~/.config/e2e/agents/zoho-sprint.mjs
```

**Alternative**: If curl doesn't work, download manually from:
```
https://github.com/e2eneworks-oss/skills/blob/main/scripts/zoho-sprint.mjs
```
...
ZOHO_SPRINT_CLIENT_ID=...
ZOHO_SPRINT_CLIENT_SECRET=...
ZOHO_SPRINT_REFRESH_TOKEN=...
```

Once user provides the content, save it:
```bash
cat > ~/.config/e2e/agents/.env << 'EOF'
[paste user's content here]
EOF
chmod 600 ~/.config/e2e/agents/.env
```
...
```bash
ls -la ~/.config/e2e/agents/zoho.json 2>/dev/null
```
...
Test the setup:
```bash
node ~/.config/e2e/agents/zoho-sprint.mjs sprints
```
...
You can now use:
  /create-issue    - Create Zoho Sprint items and branches

Configuration saved to:
  ~/.config/e2e/agents/.env (credentials, mode 0600)
  ~/.config/e2e/agents/zoho.json (project config)

Available commands:
   node ~/.config/e2e/agents/zoho-sprint.mjs sprints  - List sprints
   node ~/.config/e2e/agents/zoho-sprint.mjs items    - List items
   node ~/.config/e2e/agents/zoho-sprint.mjs projects - List projects
```
...
```bash
node ~/.config/e2e/agents/zoho-sprint.mjs use <project-name>
```

**View available projects**:
```bash
node ~/.config/e2e/agents/zoho-sprint.mjs projects
```
...
Edit `~/.config/e2e/agents/zoho.json`:
```json
{
  "current": "sre",
  "projects": {
...
```bash
node ~/.config/e2e/agents/zoho-sprint.mjs use discovery
```
...
**Symptoms**:
```
Error: ~/.config/e2e/agents/.env not found
```
...
- Check file permissions: `chmod 600 ~/.config/e2e/agents/.env`

### Script Not Found

**Symptoms**:
```
node: cannot find module ~/.config/e2e/agents/zoho-sprint.mjs
```
...
**Symptoms**:
```
Error: EACCES: permission denied, open '~/.config/e2e/agents/.env'
```
...
```bash
rm ~/.config/e2e/agents/.env
rm ~/.config/e2e/agents/.zoho-token-cache.json
...
**Credential Storage**:
- Credentials stored in `~/.config/e2e/agents/.env` with mode `0600`
...
- `/create-issue` — Create Zoho Sprint items after setup
