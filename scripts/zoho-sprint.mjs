 ...
 *
 * Usage:
 *   node zoho-sprint.mjs setup                        # Interactive OAuth setup
 *   node zoho-sprint.mjs sprints                      # List sprints
 *   node zoho-sprint.mjs items [--sprint <id>]        # List items in active/given sprint
 *   node zoho-sprint.mjs create-item <title> [opts]   # Create item in active sprint
 *   node zoho-sprint.mjs create-task <itemId> <title> # Create task under item
 *   node zoho-sprint.mjs update-status <itemId> <st>  # Update item status
 *   node zoho-sprint.mjs get <itemId>                 # Get item details
 *
 * Config: ~/.config/e2e/agents/zoho.json
 * Secrets: ~/.config/e2e/agents/.env
 */
...
const CONFIG_DIR = join(homedir(), '.config', 'e2e', 'agents');
...
function saveEnv(values) {
  ...
  content += '# Generated by: node zoho-sprint.mjs setup\n\n';
...
function getFullConfig() {
  ...
  if (!env) {
    console.error(`Error: ${ENV_FILE} not found. Run: node zoho-sprint.mjs setup`);
    process.exit(1);
  }

  if (!jsonConfig) {
    console.error(`Error: ${CONFIG_FILE} not found. Run: node zoho-sprint.mjs setup`);
    process.exit(1);
  }

  const required = ['ZOHO_SPRINT_CLIENT_ID', 'ZOHO_SPRINT_CLIENT_SECRET', 'ZOHO_SPRINT_REFRESH_TOKEN'];
  const missing = required.filter(k => !env[k]);

  if (missing.length > 0) {
    console.error(`Error: Missing env vars: ${missing.join(', ')}`);
    console.error('Run: node zoho-sprint.mjs setup');
    process.exit(1);
  }
  ...
  return {
    clientId: env.ZOHO_SPRINT_CLIENT_ID,
    clientSecret: env.ZOHO_SPRINT_CLIENT_SECRET,
...
async function cmdSetup() {
  ...
  console.error('  - Click "Add Client" → "Self Client"');
  ...
  saveEnv({
    ZOHO_SPRINT_CLIENT_ID: clientId,
    ZOHO_SPRINT_CLIENT_SECRET: clientSecret,
